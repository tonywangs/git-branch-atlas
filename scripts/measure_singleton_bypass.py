#!/usr/bin/env python3
"""Reproduce bounded loose/packed singleton-bypass comparisons (fresh processes)."""
import argparse
...
SOURCES = {'unbatched': PLAIN, 'batched': FROZEN, 'singleton': ROOT/'experiments/singleton_bypass/candidate'}


def options(case):
    return dict(max_count=32, max_bytes=256*1024*1024, seconds=600,
                max_comparisons=100000, include_groups=True, max_groups=1 if case == 'singleton-tail' else 1600,
...
def worker(args):
    ...
    extra = dict(batch_calls=0, batch_disabled_transitions=0, batch_empty_returns=0,
                 ...
                 batch_seconds=0., standalone_patch_calls=0, singleton_requests=0,
                 singleton_bypasses=0, request_sizes={})
    ...
    if args.kind != 'unbatched':
        ...
        def prefetch(budget, requests):
            ...
            extra['request_sizes'][size] = extra['request_sizes'].get(size, 0)+1
            extra['singleton_requests'] += len(requests) == 1
            extra['singleton_bypasses'] += args.kind == 'singleton' and len(requests) == 1
...
def fixture(case, seed):
    ...
    refs = {'root':root}
    count = 17 if case == 'singleton-tail' else (1 if case in ('below-cap','above-cap','singleton-small','singleton-unsupported') else 6)
    ...
    for side in ('left','right'):
        ...
        for i in range(count):
            ...
            if case in ('below-cap','above-cap'):
                ...
                if (case == 'mixed' and i%3 == 1) or case == 'singleton-unsupported':
...
def main():
    ...
    parser.add_argument('--output',type=Path,default=ROOT/'results/singleton-bypass.json')
    ...
    for source in (PLAIN,FROZEN,SOURCES['singleton']):
        assert latency.source_hashes(source) == json.loads((source.parent/'baseline.json').read_text())['files']
    assert latency.source_hashes(ROOT) in (latency.source_hashes(FROZEN), latency.source_hashes(SOURCES['singleton'])), 'unexpected production source'
                ...
                for storage in ('loose','packed'):
                    ...
                    for pair in range(1 if args.profile_only else args.repeat):
                        ...
                        for kind in order:
                            ...
                            assert row['complete'] == (case not in ('limited', 'singleton-tail')), (key,row)
    ...
    # Paired ratios use the same fixture and repetition, regardless of run order.
    decision_checks = {}
    if not args.profile_only:
        for key in fixtures:
            by_kind = {kind:{r['pair']:r['wall_seconds'] for r in rows
                            if r['workload']==key and r['kind']==kind} for kind in SOURCES}
            ratios = {label:[by_kind[a][i]/by_kind[b][i] for i in sorted(by_kind[a])]
            ...
            summary[key]['paired_runtime_ratios'] = ratios
            ratio = statistics.median(ratios['singleton_over_batched'])
            paired = {k:{r['pair']:r for r in rows if r['workload']==key and r['kind']==k} for k in SOURCES}
            decision_checks[key] = dict(
                runtime=ratio <= (0.90 if key.startswith('above-cap/') else 1.05),
                rss=summary[key]['singleton']['max_peak_rss_kib'] <= 1.10*summary[key]['batched']['max_peak_rss_kib'],
                subprocesses=all(paired['singleton'][i]['git_subprocesses'] <= paired['batched'][i]['git_subprocesses'] for i in paired['singleton']),
    ...
    evidence = dict(started_utc=started_utc, finished_utc=datetime.now(timezone.utc).isoformat(),
        cpu_count=os.cpu_count(), affinity=sorted(os.sched_getaffinity(0)) if hasattr(os,'sched_getaffinity') else None,
        environment_overrides={k:v for k,v in latency.clean_env().items() if k.startswith(('GIT_','PYTHON')) or k=='LC_ALL'},
        python=platform.python_version(),platform=platform.platform(),
        git=subprocess.check_output(['git','--version'],text=True).strip(),
        source_sha256={k:latency.source_hashes(v) for k,v in SOURCES.items()},
        harness_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        candidate_adoption_checks=decision_checks,
        candidate_meets_rule=bool(decision_checks) and all(all(v.values()) for v in decision_checks.values()),
        dependency_sha256={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in
                           (ROOT/'scripts/measure_group_latency.py',ROOT/'tests/test_patches.py',ROOT/'experiments/singleton_bypass/README.md')},
        candidate='Frozen candidate: prefetch returns empty hints for exactly one request',
