...
def main():
    ...
    if not args.profile_only:
        for key in fixtures:
            ...
            ratio = statistics.median(ratios['quarter_over_batched'])
            decision_checks[key] = ratio <= (0.90 if key.startswith('batch-overflow/') else 1.05)
    evidence = dict(python=platform.python_version(),platform=platform.platform(),
        ...
        candidate_meets_rule=bool(decision_checks) and all(decision_checks.values()),
        dependency_sha256={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in
                           (ROOT/'scripts/measure_group_latency.py',ROOT/'tests/test_patches.py',ROOT/'experiments/batching_limits/README.md')},
